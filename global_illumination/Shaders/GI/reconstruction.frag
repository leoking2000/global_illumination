...
// matrices
uniform mat4 uniform_projection_view_inv;
...
{
	vec4 p_wcs = uniform_projection_view_inv * vec4(p_ccs,1.0);
...
void main(void)
{
	...
	vec3 pos = texture(u_tex_pos, uv).xyz; // pos_wcs
	vec3 normal = texture(u_tex_normal, uv).xyz;

	vec3 extents = uniform_bbox_max - uniform_bbox_min;

	vec3 sz = textureSize(caching_data[0], 0);
    vec3 uvw = (pos - uniform_bbox_min) / extents;

    vec3 normalized_extents = extents / max (extents.x, max(extents.y,extents.z) );
	
	vec3 v_rand = vec3(0.5, 0.5, 0.5);
	vec3 v_1 = normalize(cross(normal,v_rand));
	vec3 v_2 = cross(normal,v_1);
	...
	for (int i = 0; i < 4; i++)
	{
		vec3 sdir = normal * D[i].x + v_1 * D[i].y + v_2 * D[i].z;
		vec3 uvw_new = (0.1*normal + sdir)/sz + uvw;
		
		vec3 crc_pos = uniform_bbox_min + uvw_new * extents;

		vec3 path = crc_pos - pos;
		...
		sample_irradiance = 1.0 * dotSH (normal, L00, L1_1, L10, L11, L2_2, L2_1, L20, L21, L22);
	...
	else
	{
		...
		color = DoLightingCalculations(albedo, pos, normal, mask, intensity, gi_diffuse_color);
	}
